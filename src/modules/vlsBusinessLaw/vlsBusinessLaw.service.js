...
const PDF_COLUMNS = [
  ...
  { key: "registered_date", label: "Registered", width: 70 },
  { key: "programm_date", label: "Programme", width: 70 },
  { key: "payment_status", label: "Payment", width: 64 },
  { key: "captured", label: "Captured", width: 54 },
  { key: "page_name", label: "Page", width: 88 },
  { key: "utm_source", label: "UTM", width: 64 },
  { key: "created_at", label: "Created", width: 78 },
];
...
const normalizePayload = (data) => ({
  ...
  registered_date: data.registered_date || new Date(),
  programm_date: normalizeOptionalText(data.programm_date),
...
const listRows = async (filters, tenant, options = {}) => {
  ...
  if (options.paginate) {
    return db.VlsBusinessLaw.findAndCountAll({
      ...query,
      limit: options.limit,
  ...
  return db.VlsBusinessLaw.findAll(query);
};
  ...
  new Promise((resolve, reject) => {
    ...
    const drawHeader = (pageNumber) => {
      doc.font("Helvetica-Bold").fontSize(17).fillColor("#111827").text(
        "VLS Law - Business Law (DRT & SARFAESI) Registration Report",
...
export const listVlsBusinessLawRegistrations = async (filters, tenant) => {
...
export const exportVlsBusinessLawReport = async (filters, tenant) => {
  ...
  if (filters.format === "pdf") {
    return {
      ...
      filename: `vls-business-law-registrations-${fileDate}.pdf`,
    };
  }
  ...
  return {
    ...
    filename: `vls-business-law-registrations-${fileDate}.csv`,
  };
};

export const getVlsBusinessLawSummary = async (filters, tenant) => {
  const where = await buildWhere(filters, tenant);
  ...
  const [totalRegistrations, todayRegistrations, totalAmount, paidRegistrations] = await Promise.all([
    db.VlsBusinessLaw.count({ where }),
    db.VlsBusinessLaw.count({
      where: {
        ...where,
        registered_date: {
          [Op.gte]: todayStart,
...
export const getVlsBusinessLawRegistrationById = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const record = await db.VlsBusinessLaw.findOne({ where, attributes });
  if (!record) throw createHttpError(404, "Business Law registration not found");
  return record;
};

export const createVlsBusinessLawRegistration = async (data, tenant, requestedClientKey) => {
  const tenantWhere = await resolveTenantWhere(tenant, requestedClientKey);
  return db.VlsBusinessLaw.create({
    ...normalizePayload(data),
    client_id: tenantWhere.client_id,
  });
};

export const createVlsBusinessLawPublicRegistration = async (data, clientId) =>
  db.VlsBusinessLaw.create({
    ...normalizePayload(data),
    client_id: clientId,
  });

export const updateVlsBusinessLawRegistration = async (id, data, tenant, requestedClientKey) => {
  const record = await getVlsBusinessLawRegistrationById(id, tenant, requestedClientKey);
  return record.update(normalizePayload({ ...record.toJSON(), ...data }));
};

export const deleteVlsBusinessLawRegistration = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const deleted = await db.VlsBusinessLaw.destroy({ where });
  if (!deleted) throw createHttpError(404, "Business Law registration not found");
};
