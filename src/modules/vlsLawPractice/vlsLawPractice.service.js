...
const buildFilters = (filters = {}) => {
  ...
  if (page_name) queryFilters.page_name = page_name;
...
const listRows = async (filters, tenant, options = {}) => {
  ...
  if (options.paginate) {
    return db.VlsLawPractice.findAndCountAll({
      ...query,
      limit: options.limit,
  ...
  return db.VlsLawPractice.findAll(query);
};
  ...
  new Promise((resolve, reject) => {
    ...
    const drawHeader = (pageNumber) => {
      doc.font("Helvetica-Bold").fontSize(17).fillColor("#111827").text(
        "VLS Law - Practice Enrollment Report",
...
export const listVlsLawPracticeRegistrations = async (filters, tenant) => {
...
export const exportVlsLawPracticeReport = async (filters, tenant) => {
  ...
  if (filters.format === "pdf") {
    return {
      ...
      filename: `vls-law-practice-enrollments-${fileDate}.pdf`,
    };
  }
  ...
  return {
    ...
    filename: `vls-law-practice-enrollments-${fileDate}.csv`,
  };
};

export const getVlsLawPracticeSummary = async (filters, tenant) => {
  const where = await buildWhere(filters, tenant);
  ...
  const [totalRegistrations, todayRegistrations, totalAmount, paidRegistrations] = await Promise.all([
    db.VlsLawPractice.count({ where }),
    db.VlsLawPractice.count({
      where: {
        ...where,
        registered_date: {
          [Op.gte]: todayStart,
...
export const getVlsLawPracticeRegistrationById = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const record = await db.VlsLawPractice.findOne({ where, attributes });
  if (!record) throw createHttpError(404, "Law Practice registration not found");
  return record;
};

export const createVlsLawPracticeRegistration = async (data, tenant, requestedClientKey) => {
  const tenantWhere = await resolveTenantWhere(tenant, requestedClientKey);
  return db.VlsLawPractice.create({
    ...normalizePayload(data),
    client_id: tenantWhere.client_id,
  });
};

export const createVlsLawPracticePublicRegistration = async (data, clientId) =>
  db.VlsLawPractice.create({
    ...normalizePayload(data),
    client_id: clientId,
  });

export const updateVlsLawPracticeRegistration = async (id, data, tenant, requestedClientKey) => {
  const record = await getVlsLawPracticeRegistrationById(id, tenant, requestedClientKey);
  return record.update(normalizePayload({ ...record.toJSON(), ...data }));
};

export const deleteVlsLawPracticeRegistration = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const deleted = await db.VlsLawPractice.destroy({ where });
  if (!deleted) throw createHttpError(404, "Law Practice registration not found");
};
