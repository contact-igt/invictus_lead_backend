...
const resolveTenantWhere = async (tenant, requestedClientKey, filters = {}) => {
  ...
  const clientId = await resolveClientId({
    tenant,
    requestedClientKey,
    expectedModuleKey: ANTARDRASHTI_NETRALAYA_CLIENT_KEY,
  });
...
const listLeadRows = async (filters, tenant, options = {}) => {
  ...
  if (options.paginate) {
    ...
    return db.AntardrashtiNetralaya.findAndCountAll({
      ...query,
      distinct: true,
    });
  }

  return db.AntardrashtiNetralaya.findAll(query);
};
  ...
  new Promise((resolve, reject) => {
    ...
    const drawPageFrame = (includeFilters = false) => {
      ...
      doc.font("Helvetica-Bold").fontSize(16).fillColor("#0F172A").text(
        "Antardrashti Netralaya - Lead Report",
        doc.page.margins.left,
        y,
      );
      y += 22;
...
export const listAntardrashtiNetralayaLeads = async (filters, tenant) => {
  const {
    page = 1,
    limit = 20,
  } = filters;
...
export const exportAntardrashtiNetralayaLeadReport = async (filters, tenant) => {
  ...
  if (filters.format === "pdf") {
    return {
      ...
      filename: `antardrashti-netralaya-leads-${fileDate}.pdf`,
    };
  }
  ...
  return {
    ...
    filename: `antardrashti-netralaya-leads-${fileDate}.csv`,
  };
};

export const getAntardrashtiNetralayaSummary = async (filters, tenant) => {
  const where = await buildLeadWhere(filters, tenant);
...
export const getAntardrashtiNetralayaLeadById = async (
  ...
  const record = await db.AntardrashtiNetralaya.findOne({ where });

  if (!record) {
    throw createHttpError(404, "Antardrashti Netralaya lead not found");
  }
...
export const createAntardrashtiNetralayaLead = async (
  ...
  return db.AntardrashtiNetralaya.create({
    ...normalizePayload(data),
    client_id: tenantWhere.client_id,
  });
};

export const createAntardrashtiNetralayaPublicLead = async (data, clientId) =>
  db.AntardrashtiNetralaya.create({
    ...normalizePayload(data),
    client_id: clientId,
  });
...
export const updateAntardrashtiNetralayaLead = async (
  ...
  const record = await getAntardrashtiNetralayaLeadById(
    id,
    tenant,
    requestedClientKey,
  );
...
export const deleteAntardrashtiNetralayaLead = async (
  ...
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const deleted = await db.AntardrashtiNetralaya.destroy({ where });
