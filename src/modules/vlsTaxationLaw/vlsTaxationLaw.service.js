...
const listRows = async (filters, tenant, options = {}) => {
  ...
  if (options.paginate) {
    return db.VlsTaxationLaw.findAndCountAll({
      ...query,
      limit: options.limit,
  ...
  return db.VlsTaxationLaw.findAll(query);
};
  ...
  new Promise((resolve, reject) => {
    ...
    const drawHeader = (pageNumber) => {
      doc.font("Helvetica-Bold").fontSize(17).fillColor("#111827").text(
        "VLS Law - Taxation Law Registration Report",
...
export const listVlsTaxationLawRegistrations = async (filters, tenant) => {
...
export const exportVlsTaxationLawReport = async (filters, tenant) => {
  ...
  if (filters.format === "pdf") {
    return {
      ...
      filename: `vls-taxation-law-registrations-${fileDate}.pdf`,
    };
  }
  ...
  return {
    ...
    filename: `vls-taxation-law-registrations-${fileDate}.csv`,
  };
};

export const getVlsTaxationLawSummary = async (filters, tenant) => {
  const where = await buildWhere(filters, tenant);
  ...
  const [totalRegistrations, todayRegistrations, totalAmount, paidRegistrations] = await Promise.all([
    db.VlsTaxationLaw.count({ where }),
    db.VlsTaxationLaw.count({
      where: {
        ...where,
        registered_date: {
          [Op.gte]: todayStart,
...
export const getVlsTaxationLawRegistrationById = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const record = await db.VlsTaxationLaw.findOne({ where, attributes });
  if (!record) throw createHttpError(404, "Taxation Law registration not found");
  return record;
};

export const createVlsTaxationLawRegistration = async (data, tenant, requestedClientKey) => {
  const tenantWhere = await resolveTenantWhere(tenant, requestedClientKey);
  return db.VlsTaxationLaw.create({
    ...normalizePayload(data),
    client_id: tenantWhere.client_id,
  });
};

export const createVlsTaxationLawPublicRegistration = async (data, clientId) =>
  db.VlsTaxationLaw.create({
    ...normalizePayload(data),
    client_id: clientId,
  });

export const updateVlsTaxationLawRegistration = async (id, data, tenant, requestedClientKey) => {
  const record = await getVlsTaxationLawRegistrationById(id, tenant, requestedClientKey);
  return record.update(normalizePayload({ ...record.toJSON(), ...data }));
};

export const deleteVlsTaxationLawRegistration = async (id, tenant, requestedClientKey) => {
  const where = await resolveTenantWhere(tenant, requestedClientKey, { id });
  const deleted = await db.VlsTaxationLaw.destroy({ where });
  if (!deleted) throw createHttpError(404, "Taxation Law registration not found");
};
