...
const userSchema = {
  ...
  mobile: Joi.string().trim().pattern(/^[0-9]+$/).max(20).required(),
...
export const createUserSchema = Joi.object(userSchema).unknown(false);
...
export const updateUserSchema = Joi.object({
  ...
  mobile: Joi.string().trim().pattern(/^[0-9]+$/).max(20).optional(),
}).min(1).unknown(false);
...
export const validateUserId = (req, res, next) => {
  const { error, value } = userIdSchema.validate(req.params, { abortEarly: false, convert: true });
  if (error) {
