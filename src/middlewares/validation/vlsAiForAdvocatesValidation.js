...
const vlsAiForAdvocatesRegistrationSchema = Joi.object({
  ...
  experience: Joi.string().trim().optional().allow(null, ""),
  ...
  captured: Joi.string().optional().allow(null, ""),
...
export const validateVlsAiForAdvocatesRegistration = (req, res, next) => {
  const { error, value } = vlsAiForAdvocatesRegistrationSchema.validate(req.body, {
    abortEarly: false,
    stripUnknown: true,
  });
