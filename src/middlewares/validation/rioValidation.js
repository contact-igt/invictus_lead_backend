...
const optionalService = Joi.string().trim().valid(...RIO_SERVICES).allow(null, "").optional();
const optionalBranch = Joi.string().trim().valid(...RIO_BRANCHES).allow(null, "").optional();
...
export const rioIdSchema = Joi.object({ id: Joi.number().integer().positive().required() }).unknown(false);
export const rioListSchema = Joi.object({
  page: Joi.number().integer().min(1).default(1),
  limit: Joi.number().integer().min(1).max(100).default(20),
  search: Joi.string().trim().max(255).allow("").optional(),
  service: Joi.string().trim().valid(...RIO_SERVICES).allow("").optional(),
  branch: Joi.string().trim().valid(...RIO_BRANCHES).allow("").optional(),
  utm_source: Joi.string().trim().max(255).allow("").optional(),
  start_date: Joi.date().iso().optional(),
  end_date: Joi.date().iso().min(Joi.ref("start_date")).optional(),
  _client_key: Joi.string().trim().lowercase().max(100).optional(),
}).unknown(false);
export const rioExportSchema = rioListSchema.keys({ format: Joi.string().valid("csv", "pdf").required() });
const rioContextSchema = Joi.object({ _client_key: Joi.string().trim().lowercase().max(100).optional() }).unknown(false);

const validate = (source, schema) => (req, res, next) => {
  const { error, value } = schema.validate(req[source], { abortEarly: false, convert: true });
  if (error) return res.status(400).json({ success: false, message: "Validation error", details: error.details.map((detail) => detail.message) });
  if (source === "query") Object.defineProperty(req, "query", { value, writable: true, enumerable: true, configurable: true });
